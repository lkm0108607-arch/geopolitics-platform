...
// ─── Chart Placeholder (testing without TradingView) ─────────────────────────

function TradingViewChart({ ticker, interval }: { ticker: string; interval: string }) {
  return (
    <div className="flex flex-col items-center justify-center h-full text-slate-500 text-sm gap-2 bg-slate-800/30">
      <BarChart3 className="w-10 h-10 text-slate-600" />
      <p className="text-slate-400 font-medium">KRX:{ticker}</p>
      <p className="text-xs text-slate-600">차트 영역 ({interval})</p>
    </div>
  );
}
