...
function computeTimingScore(pred: AIPrediction): number {
  ...
  else if (pred.confidence < 25) score -= 5;
...
export interface PortfolioResult {
  ...
  exitDay: number | null;        // 몇째 날에 청산 (1~7, null=데이터 없음)
  // 성적
  predictedReturn: number;
  actualReturn: number;
  wasCorrect: boolean;
...
function simulateTrade(
...
): { exitPrice: number; exitReason: ExitReason; exitDay: number; actualReturn: number } {
  if (snapshots.length === 0) {
    // 데이터 없으면 prediction_results 기반으로 fallback
    return { exitPrice: 0, exitReason: "기간종료", exitDay: 0, actualReturn: 0 };
  }

  const entryPrice = snapshots[0].close;
  if (entryPrice <= 0) {
    return { exitPrice: 0, exitReason: "기간종료", exitDay: 0, actualReturn: 0 };
  }

  const tpPrice = entryPrice * (1 + Math.abs(pick.predictedReturn) / 100);
  const slPrice = entryPrice * (1 - pick.stopLossPercent / 100);

  // 매일 TP/SL 체크 (진입 다음 날부터)
  for (let i = 1; i < snapshots.length && i <= pick.holdingDays; i++) {
    ...
    if (day.low <= slPrice) {
      ...
      return { exitPrice: slPrice, exitReason: "손절", exitDay: i, actualReturn: Math.round(ret * 100) / 100 };
    }

    // TP 체크
    if (day.high >= tpPrice) {
      const ret = Math.abs(pick.predictedReturn);
      return { exitPrice: tpPrice, exitReason: "익절", exitDay: i, actualReturn: Math.round(ret * 100) / 100 };
    }
  }
  ...
  const actualReturn = Math.round(((exitPrice - entryPrice) / entryPrice) * 100 * 100) / 100;

  return { exitPrice, exitReason: "기간종료", exitDay: lastIdx, actualReturn };
}
...
async function buildReportFromPredictions(
  ...
  // AIPrediction 형태로 변환 (timingPrediction 포함)
  const predictions: AIPrediction[] = rawPreds.map((p) => {
    // sub_model_votes에 timingPrediction이 저장되어 있을 수 있음
    const rawTiming = (p as Record<string, unknown>).timing_prediction ??
      ((p.sub_model_votes as Record<string, unknown>)?.timingPrediction) ?? null;
    ...
    return {
      ...
      subModelVotes: p.sub_model_votes as SubModelVotes,
  ...
  const portfolioResults: PortfolioResult[] = portfolio.map((pick) => {
    ...
    if (snapshots.length > 0 && snapshots[0].close > 0) {
      ...
      exitPrice = Math.round(sim.exitPrice);
